                ...
                {!! Form::model($result, array('action' => ["admin\\$controllerName@admin_update", $result->id], 'files' => true, 'id'=>'submitFrm')) !!}
                    <div class="box-body">
                        ...
                        <div class="checkbox bike-category">
                            <label >
                                {!! Form::checkbox('status', '1', ($result  && $result->required != 0)? true:null)  !!}Status Active
                            </label>
                        </div>
                        <div class="form-group" id="select_cat_div">
                            <label for="">Select Catgeories</label>
                            {!! Form::select('categories_id[]', $categories, !empty($result->categories)?$result->categories->pluck('id'):'' , [ 'class' => 'form-control categories','multiple'=>'multiple']) !!}
